...
export default class Search extends Component<Props, States> {
  ...
  openSearchInExplorer = () => {
    ...
    remote.shell.openExternal(
      `http://explorer.snowflake-net.com/transaction.html?hash=${encodeURIComponent(query)}`
    );
  };
  ...
  openInExplorer = (event: any) => {
    const hash = event.target.value;
    remote.shell.openExternal(
      `http://explorer.snowflake-net.com/transaction.html?hash=${encodeURIComponent(hash)}`
    );
  };
